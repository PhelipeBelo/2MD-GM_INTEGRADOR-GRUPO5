...
export default function PagGL() {
      ...
      if (data) {
        ...
        if (data.emUso) {
          const meusItens = data.emUso
            ...
            .map(eq => ({
                ...eq,
                nome: eq.equipamentoNome,
                codigo: eq.codigo,
                dataRetirada: eq.dataInicio
            }));
  ...
  const fetchDados = async () => {
    ...
    try {
      ...
      setToast({ type: "danger", message: "Servidor indisponível." });
    } finally {
    ...
    <>
        ...
        <div className="container-fluid px-3 px-md-4 py-3 d-flex justify-content-between align-items-center">
          <div className="d-flex align-items-center gap-2 gap-md-3">
            ...
            <div>
              ...
              <p className="small text-light opacity-75 mb-0 d-none d-md-block">Solicitação de Equipamentos</p>
            </div>
          </div>

          <div className="dropdown text-center position-relative" ref={menuRef}>
            <button className="btn btn-outline-light user-menu-button d-flex align-items-center justify-content-center gap-2 rounded-pill px-3 py-2" onClick={() => setIsMenuOpen(!isMenuOpen)}>
              <img src={user.foto} alt="Avatar" className="rounded-circle border border-light" width="32" height="32" style={{ objectFit: 'cover' }} />
              <span className="fw-semibold d-none d-md-block">{user.nome.split(' ')[0]}</span>
              <FaChevronDown />
            </button>
            <ul className={`dropdown-menu shadow-sm border-0 p-2 end-0 margem_menu ${isMenuOpen ? "show" : ""}`} style={{ minWidth: "200px", right: 0, left: "auto" }}>
      ...
      <main className="pagadmin-container container-fluid">
          ...
          <div className="col-12">
            <div className="overview-panel p-3 d-flex flex-wrap align-items-center justify-content-between gap-3">
              ...
              <div className="d-flex gap-4 text-center pe-md-4">
                <div><h4 className="mb-0 text-primary">{meusEmprestimos.length}</h4><small className="text-muted">Em posse</small></div>
                <div className="border-start mx-1"></div>
                <div><h4 className="mb-0 text-warning">{minhasSolicitacoes.filter(s => s.status === "Pendente").length}</h4><small className="text-muted">Pendentes</small></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <nav className="nav-dashboard mb-3 overflow-auto flex-nowrap">
          <button className={`tab ${abaAtiva === "catalogo" ? "active" : ""}`} onClick={() => setAbaAtiva("catalogo")}><FaSearch className="me-2" /> Catálogo Disponível</button>
          <button className={`tab ${abaAtiva === "meus" ? "active" : ""}`} onClick={() => setAbaAtiva("meus")}><FaTools className="me-2" /> Em Minha Posse</button>
          <button className={`tab ${abaAtiva === "solicitacoes" ? "active" : ""}`} onClick={() => setAbaAtiva("solicitacoes")}><FaClipboardList className="me-2" /> Minhas Solicitações</button>
        </nav>
                  ...
                  <div className="row g-2 align-items-center">
                    ...
                    <div className="col-md-8 col-12">
                      <div className="position-relative">
                        ...
                        />
                        {busca && (
                           <button className="btn btn-sm position-absolute top-50 end-0 translate-middle-y me-2 text-secondary" onClick={() => setBusca("")}><FaTimes /></button>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="col-md-4 col-12">
                      <div className="position-relative">
                         ...
                         <select 
                            className="form-select ps-5 py-2 rounded-pill bg-light border-0 cursor-pointer"
                            value={filtroMarca}
                            onChange={(e) => setFiltroMarca(e.target.value)}
                         >
                            <option value="">Todas as Marcas</option>
                            {marcasNoBanco.map((marca, index) => (
